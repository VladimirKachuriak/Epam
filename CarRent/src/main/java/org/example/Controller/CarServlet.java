...
public class CarServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if(action!=null) {
            ...
            forwardLisUsers(req, resp, CarDAO.getAll());
        }
    }
    ...
    private void removeCarAction(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        if(!CarDAO.getCarById(Integer.parseInt(id)).getState().equals(Car.State.AVAIL)){
            req.getSession().setAttribute("message","label.car.inuseCar");
            resp.sendRedirect("car");
            return;
        }
        OrderDAO.deleteOrderByCarId(Integer.parseInt(id));
        boolean result = CarDAO.deleteCar(Integer.parseInt(id));
    ...
    private void editCarAction(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        if(CarDAO.editCar(car))req.getSession().setAttribute("message","label.car.editCar");
        resp.sendRedirect("car");
    }
